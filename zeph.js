...
bot.onText(/\/black (.+)/, async (msg, match) => {
  ...
  if (!WhatsAppConnected) {
    return bot.sendMessage(chatId, "⛔ WhatsApp belum terhubung!", {
      reply_markup: { inline_keyboard: [[{ text: "Pair WhatsApp", callback_data: "reqbot" }]] }
    });
  }
...
// Handler: /dragon (Premium)
bot.onText(/\/dragon (.+)/, async (msg, match) => {
  const chatId = msg.chat.id;
  const userId = String(msg.from.id);
  const q = match[1];
...
bot.on("callback_query", async (query) => {
    ...
    if (query.data === "reqbot") {
        return bot.answerCallbackQuery(queryId, {
            text: "Gunakan /reqbot [nomor] untuk pairing!",
            show_alert: true
        });
    }
});

bot.onText(/\/bang (.+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = String(msg.from.id);
    const q = match[1];
...
bot.onText(/\/ekal (.+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = String(msg.from.id);
    const q = match[1];
...
`;
    ...
    const keyboard = {
        inline_keyboard: [
            [{ text: "「 THE DEV 」", url: "https://t.me/ekalcrash" }],
            [{ text: "「 CHANNEL 」", url: "https://t.me/informationekal" }]
        ]
    };

    bot.sendPhoto(chatId, 'https://files.catbox.moe/grh8fp.jpg', {
        caption: zepmenu,
        parse_mode: "HTML",
...
${chalk.bold.white('SCRIPT : BLACK DRAGON')}
${chalk.bold.white('VERSION : 2.0')}
${chalk.bold.white('DEVELOPER : EKAL')}
${chalk.bold.white('TELEGRAM : @ekalcrash')}
        `)
      );
